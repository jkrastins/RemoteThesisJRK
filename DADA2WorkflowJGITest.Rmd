...
```{r identify-and-remove-primers}

cutadapt <- "/RemoteThesis/cutadapt-venv/bin/python"
...
path <- "/data/jgi_data_biosphere2/expanded_data/Glocarcypl2itags/Raw Data"

# Extract sample names, assuming filenames have format: SAMPLENAME.fastq


filenames_reverse_JGI_reads_base <- sort(list.files(path, pattern = ".fastq.gz"))

filenames_reverse_JGI_reads<- grep("11162.1.194217.", filenames_reverse_JGI_reads_base, value = TRUE)

sample_names_base <- sapply(strsplit(filenames_reverse_JGI_reads, ".fastq.gz"), `[`, 1)

sample_names <- sapply(strsplit(sample_names_base, "11162.1.194217."), `[`, 2)
...
plotQualityProfile(filenames_reverse_JGI_reads[1:39])
```
...
filtered_output_JGI_reads <- filterAndTrim(fwd = filenames_reverse_JGI_reads,
                                 ...
                                 maxEE = 2, # allow w/ up to 4 errors
                                 truncLen = (200),
...
# (black lines match black points) and are generally decreasing left to right
...
metadata_in <- read.csv(paste0("data/spReport.csv"),
                          sep = ",",
                          header = TRUE,
